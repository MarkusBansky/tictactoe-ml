...
export default class Grid {
  ...
  aiMove(): void {
    const bestMoveForAI = this.minMaxRecursion(this.latestState, Main.humanPlayer);
  ...
  minMaxRecursion(state: State, player: Player): any {
    ...
    const availableSpots = state.getEmptyIndices();
    ...
    for (let i = 0; i < availableSpots.length; i++) {
      ...
      if (player == Main.aiPlayer) {
        const result = this.minMaxRecursion(state, Main.humanPlayer);
        move.score = result.score;
      } else {
        const result = this.minMaxRecursion(state, Main.aiPlayer);
        move.score = result.score;
      }
